...
class RepaymentPlanFragment : Fragment() {
    ...
    private fun calculateRepayment() {
        ...
        if (propertyPrice <= 0 || loanTerm <= 0 || deposit <=0) {
            ...
            // Display the result
            resultTextView.text = getString(R.string.fragment_repayment_plan_result_label) + formattedRepayment + getString(R.string.currency_euros)
            resultWithDepositTextView.text = getString(R.string.fragment_repayment_plan_result_with_deposit_label) + formattedRepaymentWithDeposit + getString(R.string.currency_euros)
        }
    }
}